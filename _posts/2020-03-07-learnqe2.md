...
计算流程和电子自洽计算一样:
- 1. `pw.x`:电子结构计算
- 2.1 `ph.x`:自洽计算均一网格点(q-mesh)的$C_{s\alpha,s'\beta}(q_{ijk})$
- - 2.2  `q2r.x`:傅立叶变换到实空间: $C_{s\alpha,s'\beta}(q_{ijk})$ 到$C_{s\alpha,s'\beta}(R_{lmn})$
- - 2.3  `matdyn.x`:从实空间傅立叶到q空间,**用于非自洽/插值快速计算指定q点的**$C_{s\alpha,s'\beta}(q'_{ijk})$
- 2.2 `ph.x`,`matdyn.x`:直接自洽求解$C_{s\alpha,s'\beta}(q'_{ijk})$
...
示例目录
```
LearnQE/DOC/material-for-ljubljana-qe-summer-school-master/Day-3/example1b
```

```
mpirun -np 20 pw.x -i pw.Si.in | tee result
```
