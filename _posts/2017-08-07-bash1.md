...
```
### 注意
**直接对数组赋值,并不会把数组变成变量,如果之前定义了数组,现在要重新定义最好`unset array`**
```
...
- **expr只支持整数计算**
<br> **`expr a opr b`其中opr支持`+,-,\*,/`和取余`%`**
- **只有`bc`支持浮点运算**
- `(())`内只能写整数计算的公式，写小数会报错
- 对于一些高级的计算如`sin`可以使用`awk`,见[文件处理: grep sed awk sort echo printf tail cut paste](/2018/10/17/deal-file)
...
### 更新显示内容
[Shell 小技巧 -- 原地更新终端输出信息 & 闪动效果 & 字体高亮](https://blog.csdn.net/shida_csdn/article/details/106804077)
会自动更新显示的内容,也不换行输出
```
...
## 删除文件名中的空格
```
#for file in *获得的file是包含空格的, for i in $(ls) 是不包含空格的
